#!/usr/bin/env python3
"""Portable adapter for the imported TS-RAG inference implementation."""
...
def build_command(args: argparse.Namespace) -> list[str]:
    source = Path(__file__).resolve().parent / "source" / "inference_reproduce.py"
    command = [
        args.python,
        str(source),
        "--model", "tsrag",
    ...
    if args.checkpoint:
        command.extend(["--checkpoint", str(Path(args.checkpoint).resolve())])
    return command
...
def main() -> None:
    ...
    normalize_legacy_metrics(
        ...
        method="tsrag", dataset=args.dataset, seed=args.seed,
