...
def left_pad_and_stack_1D(tensors: List[torch.Tensor]) -> torch.Tensor:
    """Local copy of the official padding helper.

    Keeping this dependency-free prevents inference-only local model loading from
    importing AutoGluon's training and S3 utilities.  The implementation is
    byte-for-byte equivalent in behavior to models.utils.left_pad_and_stack_1D.
    """
    ...
    for tensor in tensors:
        ...
        padding = torch.full(
            size=(max_len - len(tensor),),
            fill_value=torch.nan,
            device=tensor.device,
        )
        padded.append(torch.concat((padding, tensor), dim=-1))
