...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    MIXER_VARIANTS = {"official", "uniform", "raw_softmax", "distance_aware"}

    def __init__(self, config: T5Config, augment: str, mixer_variant: str = "official"):
        assert hasattr(config, "chronos_config"), "Not a Chronos config file"

        super().__init__(config)
        self.model_dim = config.d_model
        self.augment = augment
        if mixer_variant not in self.MIXER_VARIANTS:
            raise ValueError(f"Unknown mixer variant: {mixer_variant}")
        self.mixer_variant = mixer_variant
        self.retrieval_intervention = "correct"
        ...
        if 'moe' in self.augment:
            ...
            if self.mixer_variant == "distance_aware":
                # softplus(inverse_softplus(1)) == 1.  This is the only state
                # key introduced by the distance-aware variant.
                self.distance_beta_raw = nn.Parameter(torch.tensor(0.541324854612918))
    ...
    def forward(
        ...
        if self.retrieval_intervention == "zero":
            # Apply the intervention after retrieval normalization so it is a
            # true zero-information horizon rather than a constant-series token.
            retrieved_y = torch.zeros_like(retrieved_y)
        ...
        if self.augment == 'moe':
            ...
            raw_scores = torch.stack(
                [self.gate_layer(att_output[:, i, :]) for i in range(r_M + 1)],
                dim=1,
            )
            if self.mixer_variant == "uniform":
                alpha = torch.full_like(raw_scores, 1.0 / float(r_M + 1))
            elif self.mixer_variant == "official":
                alpha = F.softmax(torch.sigmoid(raw_scores), dim=1)
            elif self.mixer_variant == "raw_softmax":
                alpha = F.softmax(raw_scores, dim=1)
            elif self.mixer_variant == "distance_aware":
                if distances is None or distances.shape != (batch_size, r_M):
                    raise ValueError(
                        f"distance-aware mixer requires distances shaped {(batch_size, r_M)}, "
                        f"got {None if distances is None else tuple(distances.shape)}"
                    )
                distance_values = distances.to(device=raw_scores.device, dtype=raw_scores.dtype)
                distance_mean = distance_values.mean(dim=1, keepdim=True)
                distance_std = distance_values.std(dim=1, keepdim=True, unbiased=False)
                normalized_distance = torch.where(
                    distance_std > 1e-6,
                    (distance_values - distance_mean) / (distance_std + 1e-6),
                    torch.zeros_like(distance_values),
                ).clamp(-3.0, 3.0)
                query_distance = torch.zeros(
                    (batch_size, 1), device=raw_scores.device, dtype=raw_scores.dtype
                )
                expert_distance = torch.cat([query_distance, normalized_distance], dim=1).unsqueeze(-1)
                beta = F.softplus(self.distance_beta_raw).to(raw_scores.dtype)
                alpha = F.softmax(raw_scores - beta * expert_distance, dim=1)
            else:  # pragma: no cover - guarded in __init__
                raise AssertionError(self.mixer_variant)
            # Step 4: fuse
